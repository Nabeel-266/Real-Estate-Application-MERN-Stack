...
import { createProperty } from "../api/propertyAPI's";
...
const AddProperty = () => {
  ...
  const imagesChangeHandler = (e) => {
    ...
    if (imageFile) {
      ...
      if (!propertyDetails.images) {
        propertyFormDataChangeHandler("added", "images", [imageFile]);
      } else {
  ...
  const removeImageHandler = (e, imageUrl, index) => {
    e.preventDefault();

    if (images.length > 1) {
      setSelectedImages((prv) => prv.filter((imgUrl) => imgUrl !== imageUrl));
  ...
  const propertyFormSubmissionHandler = async (e, action) => {
    ...
    try {
      ...
      if (isPropertyDetailsOK) {
        setLoading(true);
        let propertyDoc;

        if (action === "SUBMIT FOR REVIEW") {
          propertyDoc = { ...propertyDetails, status: "pending" };
  ...
  return (
    <>
              ...
              <form className="addPropertyForm w-full flex flex-col gap-[5rem]">
                ...
                <div className="location w-full flex flex-col gap-[1.5rem]">
                  ...
                  <div className="w-full space-y-[0.8rem]">
                    <button
                      ...
                      className={`flex items-center gap-[0.5rem] text-theme-blue border-[0.2rem] p-[1rem] rounded-lg hover:shadow-lg hover:shadow-neutral-200 hover:translate-y-[-0.1rem] disabled:opacity-80 disabled:cursor-not-allowed ${
                  ...
                  <div className="w-full space-y-[0.8rem]">
                    <div className="input w-full tabletSm:w-[70%] tabletSm:min-w-[50rem] relative z-[2]">
                      <input
                        ...
                        placeholder="0"
                        className={`w-full outline-none border-[0.2rem] text-neutral-800 font-medium pl-[5.5rem] pr-[11rem] py-[0.8rem] text-[1.5rem] rounded-md focus:border-theme-blue numberInput peer/size ${
                          error && error[0] === "size"
                            ? "border-red-700"
                            : "border-neutral-300"
                        }`}
                  ...
                  <div className="w-full space-y-[0.8rem]">
                    <div className="input w-full tabletSm:w-[70%] tabletSm:min-w-[50rem] relative z-[1]">
                      <input
                        ...
                        placeholder="0"
                        className={`w-full outline-none border-[0.2rem] text-neutral-800 font-medium pl-[6.5rem] pr-[2rem] py-[0.8rem] text-[1.5rem] rounded-md focus:border-theme-blue numberInput peer/price ${
                          error && error[0] === "price"
                            ? "border-red-700"
                            : "border-neutral-300"
                        }`}
                  ...
                  <div className="w-full space-y-[0.8rem]">
                    <div className="input w-full tabletSm:w-[70%] tabletSm:min-w-[50rem] relative z-[3]">
                      <input
                        ...
                        className={`w-full outline-none border-[0.2rem] text-neutral-800 font-medium pl-[1.5rem] pr-[4rem] py-[0.9rem] text-[1.4rem] rounded-md cursor-pointer focus:border-theme-blue ${
                ...
                <div className="features w-full flex flex-col gap-[0.5rem]">
                  ...
                  <p className="text-[1.4rem] text-neutral-600 font-medium">
                    Add additional features e.g balcony, utilities etc.
                    (Optional)
                  </p>
                ...
                <div className="images w-full flex flex-col gap-[0.5rem]">
                  ...
                  <div className="w-full space-y-[0.8rem] mt-[1.5rem]">
                    <div className="input w-full tabletSm:w-[70%] tabletSm:min-w-[50rem]">
                      <label
                        htmlFor="images"
                        className={`w-full flex items-center justify-center gap-[0.8rem] outline-none border-[0.2rem] text-neutral-800 p-[1rem] text-[1.5rem] leading-[1.5rem] font-semibold rounded-md cursor-pointer hover:border-theme-blue ${
                      ...
                      <div className="w-[70%] min-w-[50rem] grid grid-cols-3 gap-[1.5rem] mt-[1.5rem]">
                        {selectedImages?.map((imageURL, index) => (
                          ...
                          >
                            ...
                            <button
                              onClick={(e) =>
                                removeImageHandler(e, imageURL, index)
                              }
                  ...
                  <div className="w-full space-y-[0.8rem]">
                    <div className="input w-full tabletSm:w-[70%] tabletSm:min-w-[50rem]">
                      <input
                        ...
                        autoComplete="off"
                        className={`w-full outline-none border-[0.2rem] text-neutral-800  font-medium px-[1.5rem] py-[1.1rem] text-[1.6rem] leading-[1.6rem] rounded-md focus:border-theme-blue ${
                          error && error[0] === "name"
                            ? "border-red-700"
                            : "border-neutral-300"
                        }`}
                ...
                <div className="buttonCont w-full flex flex-wrap items-center justify-between gap-[2rem] mt-[2rem]">
                  <button
                    onClick={(e) =>
                      propertyFormSubmissionHandler(e, "SAVE AS DRAFT")
                    }
                    className="outline-none border-[0.2rem] text-theme-blue border-theme-blue p-[1.1rem] text-[1.55rem] leading-[1.5rem] font-semibold rounded-md cursor-pointer hover:bg-theme-blue hover:text-white hover:translate-y-[-0.1rem] hover:shadow-lg hover:shadow-neutral-200 transition-all"
                  >
                    SAVE AS DRAFT
                  </button>
                  ...
                  <button
                    onClick={(e) =>
                      propertyFormSubmissionHandler(e, "SUBMIT FOR REVIEW")
                    }
      ...
      {/* Uploading Process */}
      {loading && (
        <div className="processing fixed z-[990] top-0 left-0 bottom-0 right-0 bg-[#404040b0] backdrop-blur-[0.5rem] flex items-center justify-center text-[3.5rem] leading-[3.5rem] text-white font-semibold">
          <Loader
            value="PROCESSING"
            color="#fbbf24"
            size="0.9rem"
